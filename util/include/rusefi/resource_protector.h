 ...
 * expires - in which case the resource is force-closed), after which the coordinator
 * receives the detached pointer, may safely tear down / replace the underlying object
 * and later supply a (new) instance with open().
 *
 ...
 * - free() always returns the previously attached pointer and clears the internal
 *   reference, even on timeout (forced close) - the coordinator decides whether
 *   it is safe to actually destroy the object.
...
public:
	 ...
	 * then blocks until the user count drops to zero or the timeout expires. On
	 * timeout the resource is marked force-closed: remaining users may still hold
	 * stale pointers, but no new users are admitted.
	 *
	 * In all cases the internal reference is cleared and the previously attached
	 * pointer is returned, allowing the coordinator to perform (possibly forced)
	 * cleanup. Call open() afterwards to attach an object and admit users again.
	 *
